...
export function buildV2Fixture(symbol, overrides = {}) {
  ...
  const result = {
    ...
    decision: section({ verdictClass: lowCoverage ? 'not_allocation_ready' : 'watchlist', verdictLabel: lowCoverage ? 'Not Allocation-Ready' : 'Watchlist / Conditional', scoreDisplayState: lowCoverage ? 'withhold_fundamental_score' : 'show_score_with_coverage_caveat', displayedScore: scoreVisible ? 62 : null, auditScore: 42, confidence: lowCoverage ? 28 : 61, confidenceLabel: lowCoverage ? 'low' : 'medium', allocationReadiness: lowCoverage ? 'Preliminary risk screen only' : 'Family analysis with open evidence checks', decisionSummary: thesisCopy, strongestSupport: thesisCopy, weakestArea: riskCopy, blockers: [riskCopy], nextDiligence: ['Attach current family-specific evidence.'], researchBoundary: 'Research support only.' }, lowCoverage ? 'partial' : 'available'),
